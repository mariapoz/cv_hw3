...
| `false_positive` | Модель предсказала лишний объект, которому не соответствует ни один реальный объект на изображении|
| `missed_object` | Реальный объект присутствует на изображении, но модель не смогла его обнаружить|
| `localization_error` | Класс объекта определён правильно, но bounding box расположен неточно|
| `classification_error` | Bounding box расположен достаточно хорошо, но класс объекта определён неверно|
| `classification_and_localization_error` | Ошибка одновременно и в классе, и в локализации|
